...
def get_param_list(str_param):
    ...
    print("_DEBUG_Enter get_param_list()!")
    print(str_param)
...
def get_query_string(measurement, tagValue, query_constraint, time_start, time_end):
    """
    ...
    """
    query_from = 'select v from "' + measurement + '" where p=' + "'" + tagValue + "'"  
    query_limit = ' limit 24576'    # limit of raw data
    query_time = "and time>= now()-5s"
    
    #query_time = "and (time>='" + time_start + "' AND time<='" + time_end + "') "  # time range
